...
public class GridBookmarkSearchDriveResult extends VerticalLayout { //beschissener Name
     ...
     */
    public GridBookmarkSearchDriveResult(String s, Collection<FahrerRoute> driveList) {
        if(s.isEmpty()){
     ...
     *
     */
    private void createGrid(String s, Collection<FahrerRoute> driveList){
        if(s.isEmpty()){
            throw new IllegalArgumentException("Fehler: String ist leer.");
        }
        H1 title = new H1(s);
        ...
        grid.addColumn(start -> start.getStart().getAdresse().getStrasse() + " "
        ...
        grid.addColumn(ziel -> ziel.getZiel().getAdresse().getStrasse() + " "
