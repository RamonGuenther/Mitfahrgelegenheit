...
public class BookingsView extends VerticalLayout {
    ...
    public BookingsView(DriveRouteService driveRouteService, UserService userService, MailService mailService, BookingService bookingService) {
        UI.getCurrent().setId(PageId.BOOKINGS_VIEW.label);
    ...
    private void createBookingsView() {
        ...
        gridBookings.addComponentColumn(booking -> new Anchor("/drivetogether/profil/" +
                booking.getDriveRoute().getDriver().getId(), booking.getDriveRoute().getDriver().getFullName())).setHeader("Fahrer");
